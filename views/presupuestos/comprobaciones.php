...
if(isset($_SESSION['login_gmt']))
{
	...
	$solicitudes=mysqli_query($con, "
	SELECT solicitud.autorizado, solicitud.id, cliente.nombre as cliente, solicitud.clave,presupuesto.nombrePpto,
	solicitud.aplica, solicitud.fecha, solicitud.mesqaplica,solicitud.importe, solicitud.desgloce, solicitud.cuenta,
	solicitud.formaPago, solicitud.concepto, solicitud.solicita,solicitud.status,solicitud.fechapagado,solicitud.xcomprobar,solicitud.reembolso
	FROM solicitud 
	INNER JOIN cliente ON solicitud.idCliente=cliente.id 
	INNER JOIN presupuesto ON solicitud.clave=presupuesto.clavePpto 
	INNER JOIN credenciales ON credenciales.id= '".$_SESSION["id_gmt"]."' 
	INNER JOIN comprobacion ON solicitud.id = comprobacion.no_solicitud
	GROUP BY solicitud.id ORDER BY solicitud.id;");
?>
<!DOCTYPE html>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Comprobaciones</h4>
									...
									<table class="table table-striped" id="datatable_1">
										...
										?>
											<tr>
												...
												<!-- <td><a><?php echo $fila ["autorizado"]; ?></a></td>
												<td><a><?php echo $fila ["status"]; ?></a></td>
												<td><a><?php echo $fila ["fechapagado"]; ?></a></td> -->
												<td><a><?php echo $fila ["cliente"]; ?></td>
												<td><a><?php echo $fila ["concepto"]; ?></td>
												<td><a><?php echo $fila ["nombrePpto"]; ?></a></td>
												<td><a><?php echo $fila ["clave"]; ?></a></td>
												<td><a><?php echo $fila ["solicita"]; ?></a></td>
												<td><a>$<?php echo $fila ["importe"]; ?></a></td>
												<td><a href="detalle_comprobacion.php?solicitud=<?php echo $fila["id"]; ?>" class="btn btn-primary">Ver detalles</a></td>
											</tr>
										<?php
										}
										?> 
